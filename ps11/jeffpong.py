...
debug = False  # set this to False to get less stuff printed out in the console window

## Starting with the basepong code that is provided below, make a Breakout game (a video game similar to Pong), by following the instructions in this file to alter the code we have provided.

# 1) First, make a 1-player Pong game where the right-most wall just reflects the ball back instead of scoring a point.
  # We have already changed the ball's starting position and angle in the code below. You'll only need to make changes to Game.__init__()
  # Questions to help you do this:
  # a) How can you remove the second player's paddle?
  # b) How can you make the right-hand wall deflect the ball back instead of scoring a point? (Hint: make it an instance of a different class.)

# 2) Make a 6-deep floor-to-ceiling barricade of bricks on the right side, analogous to the maze walls we created in lecture code. Test this to make sure it works before going on. 
# Some thoughts on how go about this:
  # -- how would you make one brick, with x-coordinate 100 less than the width of the window and y-coordinate 0 (at the bottom of the screen)? Where in the code would you put the call to create the brick, so that it would show up on the screen?
  # -- how would you make one whole column of bricks, using a for loop? The x position will be fixed, but the y position will get larger with each successive iteration. You can calculate how many bricks are needed (and therefore, how many times to iterate), by dividing the window height by the height of a brick.
  # -- how would you make a set of six columns? Try iterating six times. On each iteration, the x position is larger, and you use an inner for loop to make a whole column of bricks at that position.

# 3) Make bricks disappear when they are hit by the ball by making a Brick subclass of BallDeflector and switching your bricks from problem 2 to be instances of Brick instead of instances of BallDeflector.
  # Test this to see if the bricks disappear after they are hit.
  # Hints on how to do this:
  # -- Make the Brick subclass
  # -- Make the bricks be instances of the new Brick class instead of the BallDeflector class.
  # -- In the Brick class, override the deflect_ball() method:
  #    -- call the parent class' deflect_ball() method, so that the balls deflects normally
  #    -- But also remove the brick from game instance's list of game_objects. That way, when the game tells all of it's game_objects to display themselves, this brick won't display. Also, when the ball tries to see if it's run into any other game objects, it won't check with this brick any more.

# 4) Keep track of how many bricks have been hit. Every 3rd brick, increase the ball velocity.
...
class GameWindow(pyglet.window.Window):
    ...
    def __init__(self, ball_img, paddle_imgs, wall_imgs,
        ...
        #This tells Pyglet to call .update() once every fps-th of a second
...
def debug_print(string):
    '''A little convenience function that prints the string if the global debug variable is True, and otherwise does nothing'''
    if debug:
        print string

def main():
    ball_img = pyglet.resource.image('penis.png')
    paddle_imgs = [pyglet.resource.image('jeff.png'),
